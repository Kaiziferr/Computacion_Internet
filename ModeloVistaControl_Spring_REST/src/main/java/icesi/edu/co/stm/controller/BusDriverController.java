...
public class BusDriverController {
	...
	private IDriverBusService driverBusService;
	
	private boolean flag;
	
	@Autowired
	public BusDriverController(IDriverBusService driverBusService) {
		this.driverBusService = driverBusService;
	flag = false;
	}
	...
	public String showBusDriver(Model model) {
		model.addAttribute("busdrivers",  new Tmio1BusDriver());
		flag = false;
	...
	*/ 
	...
	public String saveBusDriver(@Validated(Step1.class) Tmio1BusDriver busDriver, BindingResult bindingResult,
		...
		if (!action.equals("Cancel")) {
			...
			driverBusService.save(busDriver);
		}
